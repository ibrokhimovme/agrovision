...
class User(Base, UUIDPrimaryKeyMixin, AuditMixin):
    ...
    farm_id: Mapped[UUID | None] = mapped_column(ForeignKey("farms_ref.id"), nullable=True)
...
class FarmRef(Base, UUIDPrimaryKeyMixin):
    """
    Read-only farm reference in identity-service.
    Identity-service does not own farm data — it only stores the farm_id FK.
    Populated via event consumer when FarmCreatedEvent arrives.
    """
