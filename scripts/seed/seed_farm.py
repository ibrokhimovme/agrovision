...
Farm Service Seed
==================
Creates: 1 farm, 2 buildings, 2 sections.

Farm: Toshkent Broiler Ferma
  Building 1: Broiler 1-xona      → Section A: A-sektor (production, 2500)
                                  → Section B: B-sektor (production, 2500)
  Building 2: Ozuqa ombori binosi → (storage building, no sections)

EX-03 (Building & Section Simplification, execution-v2): the former
"Karantin bloki" building / "Karantin bo'limi" section (quarantine-type)
demo data was removed per decision_log.md BMD-002 — quarantine workflows
(including this place/type) are removed entirely, not just relabeled.
"""
import asyncio
import asyncpg
from config import *
...
async def run(conn: asyncpg.Connection) -> None:
    ...
    await conn.execute("""
        INSERT INTO farms (id, name, farm_type, address, region,
                           owner_user_id, account_id, is_active, notes,
    ...
    """, FARM_ID, "Toshkent Broiler Ferma", USER_FARM_OWNER_ID,
         FARM_SETUP_DATE, USER_SUPER_ADMIN_ID, ACCOUNT_TOSHKENT_BROILER_ID)
