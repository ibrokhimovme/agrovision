...
export default function DashboardPage() {
  ...
  const [quarantineBatches, setQuarantineBatches] = useState<Batch[]>([])
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      batchService.listBatches({ farm_id: selectedFarmId, status: 'quarantine', page_size: 50 }),
      stockService.listStockItems(selectedFarmId, 1, 100),
    ])
      .then(([activeRes, quarantineRes, stockRes]) => {
        setActiveBatches(activeRes.data)
        setQuarantineBatches(quarantineRes.data)
  ...
  const totalBirdsQuarantine = quarantineBatches.reduce((sum, b) => sum + b.current_count, 0)
  const allCurrentBatches = [...quarantineBatches, ...activeBatches]
      ...
      {!selectedFarmId ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              label="Karantin"
              value={quarantineBatches.length}
              sub="Karantinda"
            ...
            />
            <StatCard
              label="Karantindagi parrandalar"
              value={totalBirdsQuarantine.toLocaleString('uz-UZ')}
              sub="Karantindagi"
            />
          </div>
          ...
          {allCurrentBatches.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
              <p className="text-gray-400 mb-3">Faol yoki karantindagi partiyalar yo'q</p>
              <Link
                to="/livestock/new"
